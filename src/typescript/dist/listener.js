...
const hyco_https_1 = tslib_1.__importDefault(require('hyco-https'));
const axios_1 = tslib_1.__importDefault(require('axios'));
const colors_1 = tslib_1.__importDefault(require('colors'));
const config = tslib_1.__importStar(
  require('./config/netPassage.multiple.json'),
);
...
if (config.NodeEnv === 'Development') {
  const httpsAgent = new hyco_https_1.default.Agent({
...
const postDataAsync = async (url, data, headers) => {
...
const displayHeader = (configs) => {
  const displayHeaders = [];
  console.clear();
  displayHeaders.push(
    `${colors_1.default.green('NetPassage')} by ${colors_1.default.green(
      'Microsoft',
    )}`,
  );
  displayHeaders.push(`${colors_1.default.green('Version: 1.0.0')}\n`);
  displayHeaders.push(
    `Azure Relay Namespace:\t\t ${colors_1.default.green(
      configs[0].namespace,
    )}`,
  );
  configs.forEach((config) => {
    ...
    displayHeaders.push(
      `Forwarding:           \t\t ${relayPath} ${colors_1.default.green(
        '-->',
      )} ${config.targetHttp}`,
    );
  });
...
const getLogMessage = (log) => {
  ...
  const contentLength = `${
    log.contentLength.length > 0
      ? // eslint-disable-next-line max-len
        `${colors_1.default.white('[Content-Length:')} ${colors_1.default.green(
          log.contentLength,
        )}${colors_1.default.white('; Data:')} ${colors_1.default.green(
  ...
  if (statusCode >= 500) {
    statusMessage += colors_1.default.red(
      `${statusCode} ${log.statusText} ${contentLength}`,
    );
  } else if (statusCode >= 300) {
    statusMessage += colors_1.default.yellow(
      `${statusCode} ${log.statusText} ${contentLength}`,
    );
  } else {
    statusMessage += colors_1.default.green(
      `${statusCode} ${log.statusText} ${contentLength}`,
    );
  }
...
azureRelayConfig.forEach((config) => {
  const listenUri = hyco_https_1.default.createRelayListenUri(
  ...
  const server = hyco_https_1.default.createRelayedServer(
    {
      server: listenUri,
      token: () =>
        ...
        hyco_https_1.default.createRelayToken(
          listenUri,
          config.policyName,
          config.policyKey,
          tokenExpired,
